...
# Config
TRADE_SIZE = float(os.getenv("BTC_SNIPER_SIZE", "1.5"))  # USDC per trade
ENTRY_SECONDS_BEFORE = 60  # Enter 60s before close
MIN_PRICE_MOVE_PCT = 0.02  # Need at least 0.02% BTC move to have conviction
MAKER_PRICE = 0.60  # Place maker order at 60¢ (profit 40¢ if wins)
MAX_MAKER_PRICE = 0.80  # Don't pay more than 80¢
...
class BTCSniper:
    ...
    def _get_direction(self) -> str:
        ...
        if change_pct > MIN_PRICE_MOVE_PCT / 100:
            return "up"
        elif change_pct < -MIN_PRICE_MOVE_PCT / 100:
            return "down"
        return "flat"
